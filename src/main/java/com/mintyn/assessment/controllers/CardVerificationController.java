...
public class CardVerificationController {
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Successfully retrieved card verification stats",
                    ...
                    @Schema(implementation = CardStatsResponse.class)))),
            @ApiResponse(responseCode = "500", description = "Internal Server Error", content = @Content(array =
    ...
    public ResponseEntity<CardVerificationResponse> verifyCard(@PathVariable @Pattern(regexp = "^[0-9]{13,19}$",
    message = "Invalid card number format. Please enter a numeric string with a length between 13 and 19 characters.")
                                                               String cardNumber) {
