...
public class CardVerificationServiceImpl implements CardVerificationService {
    ...
    public CardStatsResponse getCardVerificationStats(int start, int limit) {
        ...
        return CardStatsResponse.builder().success(true).start(start).limit(limit).size(cardVerifications
                .getTotalElements()).payload(getCardNumberToHitCountAsMap(cardVerifications)).build();
    }
    ...
    public CardVerificationResponse verifyCard(String cardNumber) {
        ...
        if (cardVerificationDetails != null) {
            ...
            CardVerificationPayload cardVerificationPayload = CardVerificationPayload.builder()
                    .scheme(cardVerificationDetails.getScheme()).type(cardVerificationDetails.getType())
                    .bank(cardVerificationDetails.getBank()).build();
        ...
        // Save to the database and cache
        cardVerificationDetails = CardVerification.builder().cardNumber(cardNumber).scheme(cardScheme).type(cardType)
                .bank(issuerName).hitCount(1).build();
        cardVerificationCache.put(cardNumber, cardVerificationDetails);
        cardVerificationRepository.save(cardVerificationDetails);
        CardVerificationPayload cardVerificationPayload = CardVerificationPayload.builder().scheme(cardScheme)
                .type(cardType).bank(issuerName).build();
        return CardVerificationResponse.builder().success(true).payload(cardVerificationPayload).build();
    }

    private Map<String, String> getCardNumberToHitCountAsMap(Page<CardVerification> cardVerifications) {
        return cardVerifications.stream().collect(Collectors.toMap(CardVerification::getCardNumber,
                verification -> String.valueOf(verification.getHitCount())));
    }
    ...
    private BinResponse getBinDetails(String bin) {
        ...
        try {
            ResponseEntity<BinResponse> response = webClient.get().uri(appConfigurations.getBinDetailApiUrl()+bin)
                    .retrieve().toEntity(BinResponse.class).timeout(Duration.ofSeconds(10))
                    .block();
            if (response != null) {
